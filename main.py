...
@app.route('/api/trigger/<element>/<action>', methods=['GET', 'POST'])
def trigger_element(element, action):
    global lower_third_state, ticker_state, logo_state, title_card_state, social_bar_state, breaking_news_state
    
    elements = {
        'lowerthird': (lower_third_state, 'state_update'),
        'ticker': (ticker_state, 'ticker_update'),
        'logo': (logo_state, 'logo_update'),
        'titlecard': (title_card_state, 'titlecard_update'),
        'social': (social_bar_state, 'social_update'),
        'breaking': (breaking_news_state, 'breaking_update')
    }
    
    if element not in elements:
        return jsonify({'error': 'Invalid element', 'valid': list(elements.keys())}), 400
    
    state, event = elements[element]
    ...
    if action == 'show':
        ...
        return jsonify({'error': 'Invalid action', 'valid': ['show', 'hide', 'toggle']}), 400
    
    socketio.emit(event, state)
    return jsonify({'element': element, 'action': action, 'visible': state['visible']})

@app.route('/api/trigger/lowerthird/set', methods=['GET', 'POST'])
def set_lower_third():
    global lower_third_state
    line1 = request.args.get('line1') or (request.json or {}).get('line1')
    line2 = request.args.get('line2') or (request.json or {}).get('line2')
    
    if line1:
        lower_third_state['line1'] = line1
    if line2:
        lower_third_state['line2'] = line2
    
    socketio.emit('state_update', lower_third_state)
    return jsonify(lower_third_state)

@app.route('/api/trigger/breaking/set', methods=['GET', 'POST'])
def set_breaking():
    global breaking_news_state
    text = request.args.get('text') or (request.json or {}).get('text')
    
    if text:
        breaking_news_state['text'] = text
    
    socketio.emit('breaking_update', breaking_news_state)
    return jsonify(breaking_news_state)

@app.route('/api/trigger/hideall', methods=['GET', 'POST'])
def hide_all():
    global lower_third_state, ticker_state, logo_state, title_card_state, social_bar_state, breaking_news_state
    ...
    return jsonify({'status': 'all hidden'})
